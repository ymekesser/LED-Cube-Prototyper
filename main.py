...
def blinking_edges():
    def is_edge(pos):
        return pos.count(0) >= 2 or pos.count(led_cube.x_count - 1) >= 2 or (0 in pos and led_cube.x_count - 1 in pos)
...
def expanding_sphere():
    ...
    d = 2
    s = 0.01
    offset = tuple(numpy.divide((led_cube.x_count, led_cube.y_count, led_cube.z_count), 2))
    while (True):
        led_frame = []
        for i in range(led_cube.get_led_count()):
